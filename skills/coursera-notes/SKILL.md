...
# Course → Notes (Markdown / Obsidian · Notion · Google Docs)
...
1. **Pick the destination first.** Ask the user where notes go if not already stated: **Markdown / Obsidian** (default), **Notion**, or **Google Docs**. This decides which MCP/tools you need and how the tree is built (see [Destinations](#destinations)).
2. **Playwright MCP** connected + the destination's tool available: Notion MCP (Notion), Google Drive MCP (Google Docs), or just the filesystem (Markdown / Obsidian — no MCP needed).
...
4. **Target location** confirmed with the user — the folder path (Obsidian vault or `./notes`), the Notion parent page, or the Google Drive folder.
...
Create the hierarchy at the chosen destination — one node per level (Course → Section → [Lesson] → Item), parenting each under the one above. **How** you create a node depends on the destination; see [Destinations](#destinations) for the exact tool and nesting per destination. The tree *shape* below is the same everywhere:
- Course → under the user's chosen parent (folder · Notion page · Drive folder)
...
|-------------|--------|---------------------|-------|
| **Markdown / Obsidian** (default) | Nested folders + one `.md` per node, plus `00-index.md` per folder | `Write` tool | Obsidian is *just* a markdown renderer — these files ARE an Obsidian vault. Write them into the user's vault path if they have one, else `./notes`. Number-prefix names to sort. |
...
**Markdown / Obsidian is one destination.** There is no separate "Obsidian export" — Obsidian reads plain markdown. Ask only *where* the folder goes (vault path vs. `./notes`), not which of the two formats. Links: plain relative markdown (`[title](01-item.md)`) — renders in Obsidian, GitHub, and every other editor. Use `[[wikilinks]]` only if the user explicitly wants them (then follow the `obsidian-vault` skill for vault conventions).

Folder/file layout for markdown:
```
notes/<Course>/
...
| Step | Tool | Note |
|------|------|------|
| Pick destination | ask user | Markdown / Obsidian (default) · Notion · Google Docs (prereq 1) |
...
- **Treating Obsidian as a separate export format** → it renders plain markdown. Write markdown files into the vault folder; nothing else changes.
