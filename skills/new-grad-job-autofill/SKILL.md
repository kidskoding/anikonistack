---
name: new-grad-job-autofill
description: Autofill and submit a full-time new-grad / entry-level SWE job application in the browser (Playwright MCP), then log it to Notion. Use for any job posting detected as new-grad or entry-level full-time.
---

# New-Grad Job Autofill

Fill and submit one full-time new-grad application. Auto-submit ON — no confirmation needed before submitting.

## Term answers (override anything else)
- Availability: **after May 2027** (flexible)
- Graduation date to report: **May 2027** (matches résumé; new-grad forms only)
- Current student: **No** (not for these forms); skip internship-only questions
- Résumé to upload: **`resume/resume-templates/anirudh-resume-summer-2026-swe-2-5.docx`** (.docx is fine, never convert to PDF)
...
Use the term-appropriate tracker per project CLAUDE.md (new-grad roles: ask the user which tracker if none exists).
Row: **Name/Position** = `<Role> (<Team/area> - New Grad) - <Company>` · **Application Status** = `In progress` (exact casing) · submit date in **Other**. Skip silently if Notion MCP unavailable.

## Report
One line: `SUBMITTED — <Company>, <Role> (New Grad), Notion: yes/no` or `BLOCKED — <Company>, <Role>: <reason>`.
